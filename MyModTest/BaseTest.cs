...
namespace Handelabra.Sentinels.UnitTest
{
    public class BaseTest
    {
        ...
        protected HeroTurnTakerController marisa { get { return FindHero("Marisa"); } }
        protected AzureTouhou4.Marisa.MiniHakkeroCardController miniHak
        {
            get { 
                AzureTouhou4.Marisa.MiniHakkeroCardController miniHak = (AzureTouhou4.Marisa.MiniHakkeroCardController)FindCardController("MiniHakkero");
                if (miniHak.Card.IsInPlay)
                {
                    return miniHak;
                }
                else
                { 
                    return (AzureTouhou4.Marisa.MiniHakkeroCardController)FindCardController("MiniHakkeroCoolingElementalFurnace");
                }                
            }
        }
        ...
        protected void StuffMiniHak(HeroTurnTakerController marisa, int height)
        {
            marisa.HeroTurnTaker.MoveAllCards(marisa.HeroTurnTaker.Hand, marisa.HeroTurnTaker.Deck);

            //This should already be more than enough cards to make this work
            Card[] ones = marisa.FindCardsWhere((c) => !c.DoKeywordsContain("catalyst") && c.InstanceIndex == 0 && c.Location == marisa.HeroTurnTaker.Deck).ToArray();
            Card[] twos = marisa.FindCardsWhere((c) => c.DoKeywordsContain("catalyst") && c.InstanceIndex == 0).ToArray();

            int tTwos = height / 2;
             if (tTwos > twos.Length)
            {
                tTwos = twos.Length;
            }

            int tOnes = height - tTwos * 2;
            if (tOnes > ones.Length) throw new Exception("insufficient Cards to reach required height");
           
            //doing it this way to reduce unnecessary logging
            MoveCards(marisa, twos.Take(tTwos).Concat(ones.Take(tOnes)).ToArray(), miniHak.Card.UnderLocation);
        }
